...
# AlbertKe, HW2 Modified 11 -> 3 (Original was 32 for CamVid, but our dataset has 11 classes before remapping)
num_class =  11 # Set to 11 for training on original classes
...
class CamVidDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # AlbertKe, HW2 Added (Original remapping for 3 classes - REMOVE FOR 11-CLASS TRAINING)
        # label[label==2] = 1
        # label[label==3] = 1
        # label[label==4] = 1
        # label[label==5] = 1
...
def save_result_comparison(input_np, output_np):
    ...
    # The input output_np is now expected to be already remapped to 3 classes (0, 1, 2).
    # The following block is for 3-class visualization.
    for i in range(256):
        for j in range(256):
            if output_np[i,j] == 0:
                im_seg_RGB[i,j,:] = [128, 128, 128] # Class 0: Sky (or general background)
            elif output_np[i,j] == 1:
                im_seg_RGB[i,j,:] = [128, 0, 0]     # Class 1: Background/Static elements (e.g., Building, Road, Tree, etc.)
            elif output_np[i,j] == 2:
                im_seg_RGB[i,j,:] = [64, 0, 128]    # Class 2: Moving objects/Actors (e.g., Car, Pedestrian, Bicyclist)
...
def val(epoch):
    ...
    # Define the 11 to 3 class remapping function
    def remap_to_3_classes(arr):
        remapped_arr = arr.copy()
        remapped_arr[remapped_arr==2] = 1
        remapped_arr[remapped_arr==3] = 1
        remapped_arr[remapped_arr==4] = 1
        remapped_arr[remapped_arr==5] = 1
        remapped_arr[remapped_arr==6] = 1
        remapped_arr[remapped_arr==7] = 1
        remapped_arr[remapped_arr==11] = 1 # Assuming 11 is 'Unlabelled'
        remapped_arr[remapped_arr==8] = 2
        remapped_arr[remapped_arr==9] = 2
        remapped_arr[remapped_arr==10] = 2
        # Class 0 remains 0
        return remapped_arr
    ...
    for iter, batch in enumerate(val_loader): ## batch is 1 in this case
        ...
        # Get predicted class indices (0-10)
        pred_11_classes = output.data.max(1)[1].cpu().numpy() # N, H, W

        # Remap predictions to 3 classes for evaluation and visualization
        pred_3_classes = remap_to_3_classes(pred_11_classes)

        # Get target labels (0-10) and remap to 3 classes for consistent evaluation
        target_11_classes = batch['l'].cpu().numpy() # N, H, W
        target_3_classes = remap_to_3_classes(target_11_classes)

        # only save the 1st image for comparison
        if iter == 0:
            print('---------iter={}'.format(iter))
            # generate images using the 3-class remapped prediction
            image = pred_3_classes[0,:,:]
            save_result_comparison(batch['X'], image) # pass 3-class remapped image


        # Evaluate using the 3-class remapped predictions and targets
        # num_class for iou and pixel_acc should be 3 now
        num_class_for_eval = 3
        for p, t in zip(pred_3_classes, target_3_classes):
            total_ious.append(iou(p, t, num_class_for_eval))
...
def iou(pred, target, n_class_eval):
    ious = []
    for cls in range(n_class_eval):
        pred_inds = pred == cls
        target_inds = target == cls
        intersection = pred_inds[target_inds].sum()
        union = pred_inds.sum() + target_inds.sum() - intersection
        if union == 0:
            print(f"Warning: Union is 0 for class {cls}") # Added print statement
